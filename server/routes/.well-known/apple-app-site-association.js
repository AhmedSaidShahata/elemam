...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (host === "localhost:3000" || host === "127.0.0.1:3000") {
      apiUrl = `${staticUrl}/mobile-links/ios`;
    } else {
      const config = useRuntimeConfig();
      const urlEnv = config.public.envUrl;
      apiUrl = `${urlEnv}/${mainDomain}/mobile-links/ios`;
    }
    ...
    console.error("AASA fetch failed:", err.message);
    setResponseHeader(event, "Content-Type", "application/json");
    event.node.res.statusCode = 500;
    return { error: "Failed to load apple-app-site-association" };
  }
});
